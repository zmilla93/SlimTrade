...
public class DesignerConfigWindow extends CustomDialog {
    ...
    private void installNinjaDataset() {
        ZLogger.log("Downloading all datasets from poe.ninja, saving to '" + SaveManager.getNinjaDirectoryPath() + "'...");
        ...
        for (NinjaEndpoint endpoint : NinjaEndpoint.values()) {
            for (PathOfExileLeague league : PathOfExileLeague.values()) {
                ...
                executor.submit(() -> {
                    ...
                    BufferedWriter writer = ZUtil.getBufferedWriter(SaveManager.getNinjaDirectoryPath().resolve(league + File.separator + endpoint.type + ".json").toString());
                    try {
